...
if [ $help == True ]; then
  details $script
  echo2 "   fit a set of free energy files as a function of volume for different T"
  ...
  echo2 "   typical script sequence:"                                   \
        ...
        "     \033[1mgetFqhFit.sh\033[0m                                   (cmpc01)"  \
...
latType = \"bcc\";                      (* bcc, fcc, hcp, or size of supercell (2,3,4,...; needed for defects) *)
...
baseName = "$base";    (* base names for the free energy files; followed by aLat WITHOUT UNIT (e.g. no angstrom) *)
...
<<parameters.math;
fitF[aLats,cBya,latType,s,Vorder,#,suffix]&/@baseName;
EOF
