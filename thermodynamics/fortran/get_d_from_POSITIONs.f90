...
PROGRAM getDistanceFromPOSITIONs

      implicit none
      INTEGER i,j,nat,nmd
      REAL(8) dummy
      REAL(8) dist,distlong,cell(3,3),delta(3),veclong(3)
      REAL(8),allocatable :: coords(:,:), eq(:,:), eqvec(:,:,:)
      integer,allocatable :: map(:,:)

      ! get number of atoms
      open(unit=10,file='equilibrium',status='old',err=99)
      nat = 0
  ...
  20  continue
      ...
      allocate(coords(3,nat),eq(3,nat),map(nat,nat),eqvec(3,nat,nat))

      call readMap(nat,map,'mapping')
      call readCoords(3,cell,'cell')
      call readCoords(nat,eq,'equilibrium')

      DO i=1,nat
        call toReduced(eq(:,i),cell)
      ENDDO

      DO i=1,nat
        DO j=1,nat
         if (map(i,j).NE.-1) then
           call periodicDiffVec(eq(:,map(i,j)),eq(:,i),delta)
           call toCartesian(delta,cell)
           dist = sqrt(sum(delta**2))
           eqvec(1,i,j) = delta(1)/dist
           eqvec(2,i,j) = delta(2)/dist
           eqvec(3,i,j) = delta(3)/dist
         else
           exit
         endif
      ...
      open(unit=11,file="distances",status="unknown")
      open(unit=12,file="distances_long",status="unknown")

      DO
       DO i=1,nat
         read(10,*,end=30) coords(1,i),coords(2,i),coords(3,i)
       ...
       DO i=1,nat
        DO j=1,nat
         if (map(i,j).NE.-1) then
           ...
           call toCartesian(delta,cell)
           dist = sqrt(sum(delta**2))
           veclong(:) = dot_product(eqvec(:,i,j),delta(:))*eqvec(:,i,j)
           distlong = sqrt(sum(veclong**2))
           write(11,*) dist
           write(12,*) distlong
         else
           exit
  ...
  99  continue
      write (*,*) "ERROR: cannot open or empty file equilibrium"
      stop
END
