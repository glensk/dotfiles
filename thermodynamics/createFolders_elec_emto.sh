...
if [ $help == True ]; then
  ...
  echo2 "   typical script sequence"                                                                 \
        ...
        "    getFqhFit.sh                            (cmpc01)"                                       \
        "    \033[1mcreateFolders_elec_emto.sh\033[0m -a           (in separate folder; adjust parameters.dat)" \
        "    \033[1mcreateFolders_elec_emto.sh\033[0m"                                               \
        "    submit.sh run.emto.ser.cluster.workdir  (or the local version)"                         \
        "    collectEMTO.sh                          (when finished and inside NxNxNkp_XXeV folder)" \
...
if [ $genPar == True -o $all == True ]; then
  ...
  echo "
aLats=3.22 3.24 3.26    # Angstrom (typically from Veq at T=0K up to Veq at T^melt)
sigmas=0.01 0.03 0.05   # smearing in eV (typically 0.01 up to T^melt in eV)
...
ZMSH...= F NZ1..= 16 NZ2..= 24 NZ3..= 24 NRES.=  4 NZD..=200
...
sigmas=`get sigmas`; changeInp=`get changeInp`; spinType=`get spinType`; moment=`get moment`;
checkInput "$aLats" "$kp" "$sigmas" "$spinType" "$elem" "$type"
...
for a in $aLats; do for s in $sigmas; do
  f=$kp\x$kp\x$kpc\kp$addString/$a\_$s
...
if [ $cc != 10 ]; then error "inp_kgrn template wrong"; fi
...
for a in $aLats; do for s in $sigmas; do
  ...
  # transform sigma
  sigmaStr=`echo $s | awk '{printf "%6.1f",$1*11604.5061378109385}'` # also eV to Kelvin
