...
[ "$reftype" != "hesse" ] && [ "$reftype" != "extern"  ] && echored "ERROR: reftype:$reftype: in parameters.dat is neither hesse nor extern. Create parameters.dat using `basename $0` -p -o and adjust it" && exit
...
for a in $aLats; do for t in $temps; do for l in $lambdas; do for seedi in `seq 1 $nSeeds`;do
    ...
    cp POTCAR $f/POTCAR
    cp potential $f/potential
    cp calculate_energy_and_forces $f/calculate_energy_and_forces
    cp in.file $f/in.file
    #cp lmp_serial $f/lmp_serial
    cp lammps_must_be_serial $f/lammps_must_be_serial
    cp external_script_additional_input_files $f/external_script_additional_input_files
    cp rotate_coords.x $f/rotate_coords.x
    cp rotate_forces.x $f/rotate_forces.x
    cp scratch_path $f/scratch_path
