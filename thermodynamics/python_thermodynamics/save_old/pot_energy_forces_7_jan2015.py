...
class pot_energy_forces_class( object ):
    ...
    def get_to_ti_direction_vecs_fcc(self):
        ...
        if self.shell == 3:
            if self.vec0[0] != self.vec0[1]:
                self.todirection0 = np.cross(self.vec0,np.array([-self.vec0[0],-self.vec[1],0.0]))
            if self.vec0[0] != self.vec0[2]:
                self.todirection0 = np.cross(self.vec0,np.array([-self.vec0[0],0.0,-self.vec[2]]))
            if self.vec0[1] != self.vec0[2]:
                self.todirection0 = np.cross(self.vec0,np.array([0.0,-self.vec0[1],-self.vec[2]]))
    ...
    def get_to_ti_direction_vecs_fcc_old(self):
        self.topdirectionx = np.array([0.0,0.0,0.0])  # those are the directions in which the forces act on to / ti part of vector
        self.topdirectiony = np.array([0.0,0.0,0.0])
        self.topdirectionz = np.array([0.0,0.0,0.0])
        self.topdirectionxnorm = 0.0  # those are the directions in which the forces act on to / ti part of vector
        ...
        if self.vec0[0] != 0.0 and self.shell == 2:
            self.topdirectionx = np.array([0.0,1.0,0.0])
            self.tipdirectionx = np.array([0.0,0.0,1.0])
        if self.vec0[1] != 0.0 and self.shell == 2:
            self.topdirectionx = np.array([1.0,0.0,0.0])
            self.tipdirectionx = np.array([0.0,0.0,1.0])
        if self.vec0[2] != 0.0 and self.shell == 2:
            self.topdirectionx = np.array([1.0,0.0,0.0])
            self.tipdirectionx = np.array([0.0,1.0,0.0])
        ...
        #    self.tipdirectionx = np.array([0.0,0.0,1.0])  # minus xdir  will also be correct
        ...
        #    self.tipdirectionx = np.array([0.0,1.0,0.0])  # minus xdir  will also be correct
        #    if self.vec0[0] == 0.0 and self.shell == 2: # vec in y-z ebene
        #        self.topdirectiony = np.array([1.0,0.0,0.0])  # minus xdir  will also be correct
        #    if self.vec0[1] == 0.0 and self.shell == 2: # vec in y-z ebene
        #        self.topdirectiony = np.array([0.0,1.0,0.0])  # minus xdir  will also be correct
        ...
        if self.shell == 1:
            if self.topdirectionxnorm == 0.0 or self.topdirectionynorm == 0.0 or self.topdirectionznorm == 0.0 or \
                ...
                sys.exit("shell 1: t{o,i}pdirection{x,y,z} not found")
        ...
        #        print "tipdirectiony:",self.tipdirectiony
        #        print "tipdirectionz:",self.tipdirectionz
        #        sys.exit("sehll 2: t{o,i}pdirection{x,y,z} not found")
        ...
        if  type(self.u_topx) != bool or \
            ...
            type(self.u_topz) != bool:
                if self.topdirectionxnorm == 0.0 or self.topdirectionynorm == 0.0 or self.topdirectionynorm == 0.0:
                    ...
                    print "self.topdirectionxnorm:",self.topdirectionxnorm
                    print "self.topdirectionynorm:",self.topdirectionynorm
                    print "self.topdirectionznorm:",self.topdirectionznorm
        ...
        if  type(self.u_tipx) != bool or \
            ...
            type(self.u_tipz) != bool:
                if self.tipdirectionxnorm == 0.0 or self.tipdirectionynorm == 0.0 or self.tipdirectionynorm == 0.0:
                    print "shell:",self.shell
                    print "self.tipdirectionxnorm:",self.tipdirectionxnorm
                    print "self.tipdirectionynorm:",self.tipdirectionynorm
                    print "self.tipdirectionznorm:",self.tipdirectionznorm
    ...
    def get_to_ti_energy_forces(self, contr = False):
        ...
        def project_vec_into_original_cell(vec,\
        ...
        pot = False
        if contr == 'to':
            #vec_to_ti = utils.project_vector(self.longvec,self.topdirectionx) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
            vec_to_ti = utils.project_vector(self.longvec,self.todirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
            potparam = self.u_topx

        if contr == 'ti':
            #vec_to_ti = utils.project_vector(self.longvec,self.tipdirection) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
            vec_to_ti = utils.project_vector(self.longvec,self.tidirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
            potparam = self.u_tipx

        if pot == False and potparam == False:
            return

        vec_to_ti_length = np.linalg.norm(vec_to_ti)
        if vec_to_ti_length == 0.0:
            vec_to_ti_normalized_corr_direction = np.array([0.0,0.0,0.0])
        else:
            vec_to_ti_normalized_corr_direction = vec_to_ti/vec_to_ti_length
        ...
        # b)
        vec_orthbasis_b = utils.project_vector(vec_to_ti,t11_norm)
        ...
        if abs(forces_to_ti[old_i]) > 0.00001:
            if old_i != new_i:
                if forces_to_ti[old_i]/forces_to_ti_orthbasis_a[new_i] > 1.3:
                    printinfo()
        ...
        #@    if True and self.shell == 2:
        #@        print "TI:",self.vec0_curr,self.longvec,"||",vec_ti_normalized,"tipnew_norm:",t11_norm,"---> oldF:",forces_ti,"newF_a:",forces_ti
        ...
        self.forcestrantipx[self.indi] = self.forcestrantipx[self.indi] + self.ftipx #+ self.ftipx
        self.energytrantipx[self.indi] = self.energytrantipx[self.indi] + self.etipx #+ self.etipx
        self.forcestrantipy[self.indi] = self.forcestrantipy[self.indi] + self.ftipy #+ self.ftipy
        self.energytrantipy[self.indi] = self.energytrantipy[self.indi] + self.etipy #+ self.etipy
        self.forcestrantipz[self.indi] = self.forcestrantipz[self.indi] + self.ftipz #+ self.ftipz
        self.energytrantipz[self.indi] = self.energytrantipz[self.indi] + self.etipz #+ self.etipz
        return
        return
