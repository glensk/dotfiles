...
def space_fft_to_powerspectrum(space_fft, qpoint, mdstepstocalcin = False, dt = False, appendlast=True,execute_timeinversion=False, print_single_qpoints=False,idxprint=0,idxprintmax=0, check_qpoint_for_crossing = False, args = False):
    ...
    for idx,mdsteps in enumerate(mdstepstocalc):
        ...
        if check_qpoint_for_crossing == True:
            #np.savetxt("kkkk.dat",ps)
            # at this point ps is not the original powerspectrum anymore ....
        ...
        if True and mdsteps > 999:
            ...
            if args.verbose:
                print "###########################################################################################################################"
                print "################################## min----------1",sigmamin, fqmin,ltmin,ltminmin,ltminmax,xmaxwritemin,minsmoothfunc
            ...
            if args.verbose:
                print "###########################################################################################################################"
                print "################################## good---------2",sigmagood, fqgood,ltgood,ltgoodmin,ltgoodmax,xmaxwritegood,goodsmoothfunc
...
if __name__ == '__main__':
    ...
    if args.make_space_fft_from_py or args.make_space_fft_from_lammpslog or args.make_power_spectrum:
        for idx,qpoint in enumerate(qpoints_all): # fuer alle qpunkte in der liste
            ...
            print "type(filename):",type(filename),"args.make_space_fft_from_py:",args.make_space_fft_from_py
            ...
            if os.path.isfile(file_to_load) != True:
                #if os.path.isfile("POSITIONs"):  # here we can savly assume those are from (relatively short) DFT runs

                sys.exit("file "+file_to_load+" does not exist! Run this skript with -fftpy or -fftc first!")
