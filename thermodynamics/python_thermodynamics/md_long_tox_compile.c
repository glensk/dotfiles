...
#define mass_element (63.546)

// 900K GGA
#define alat_lattice (3.75)
#define alat_morse (3.644)
#define a_mor (1.995234493330231)
#define D_mor (0.12707444299373233)
#define ktr_tox (0.0)
//////////////////////////// end definitions //////////////////////////////
...
void write_out_info(double T,double dt,int zeitschritte, int l,int read_pos,int read_pos0,int read_uoutcar,int verbose,int write_analyze,const char *filename_in_positions,const char *filename_in_positions0, const char *filename_in_hesse,int read_hesse,int read_forces,int columns, int columns0, const char *filename_in_uoutcar,int evolve_md_on_hesse,int write_positions_forces,int write_positions) {
	...
	fprintf(anmerkung,"alat_lattice   : %-20.10f\n",alat_lattice);
	fprintf(anmerkung,"alat_morse     : %-20.10f\n",alat_lattice);
	fprintf(anmerkung,"d0             : %-10.10f\n",d0);
	fprintf(anmerkung,"a0             : %g (4294967296./N)\n",a0);
	fprintf(anmerkung,"r0_mor         : %g\n",r0_mor);
	fprintf(anmerkung,"r0_eq_morse    : %g\n",r0_eq_morse);
	fprintf(anmerkung,"a_mor          : %-20.10f\n",a_mor);
	fprintf(anmerkung,"D_mor          : %-20.10f\n",D_mor);
	...
	printsep("");
	printf("N              : %d\n",N);
	printf("alat_lattice   : %-20.10f\n",alat_lattice);
	printf("alat_morse     : %-20.10f\n",alat_lattice);
...
void write_dudl(int step, FILE *file_out_dudl,FILE *file_out_dudl_av,int read_hesse) {
    // step, u,
    //printf("p:a\n");
    //printf("p:a %d\n",step);
    ...
    //printf("p:step\n");
    fprintf(file_out_dudl_av,"%-8d %8.4f   %8.2f     %8.2f  %8.2f     %8.1f  %8.2f %8.2f %8.2f %8.2f %8.3f %8.3f\n",\
            step,\
...
void write_analyze_positions(int i,FILE *file_out_check_dist_xyz,FILE *file_out_check_dist_r) {
    ...
    double dx,dy,dz,rr;
		for (j=0;j<atoms;j++) {  // schleife ueber alle atome
		    ...
		    dy = (pos[j].y-pos0[j].y)/a0*alat_lattice;
...
void calculate_forces_energy_la(double dt,int verbose) {
	...
	double x,y,z,r,f,fx,fy,fz,dum,a0ktr_par,dtmfx,dtmfy,dtmfz,dumm1; //,energy;
...
int main(int argc,char *argv[]){
    ...
    const char *filename_in_positions = "/Users/glensk/Dropbox/Albert/Understanding_distributions/ti/Cu/30__PTS_dosall_from_2x2x2sc__LON_quer_3x3x3kp_all_morse_LONADD__NO_LON2_NONE_LON2ADD__NO_TOX_NONE_TOY_NONE_TOZ_NONE/lambda1.0//POSITIONs";
    const char *filename_in_positions0 = "/Users/glensk/Dropbox/Albert/Understanding_distributions/ti/Cu/30__PTS_dosall_from_2x2x2sc__LON_quer_3x3x3kp_all_morse_LONADD__NO_LON2_NONE_LON2ADD__NO_TOX_NONE_TOY_NONE_TOZ_NONE/lambda1.0//POSITIONs";
    const char *filename_in_hesse = "/Users/glensk/Dropbox/Albert/Understanding_distributions/ti/Cu/30__PTS_dosall_from_2x2x2sc__LON_quer_3x3x3kp_all_morse_LONADD__NO_LON2_NONE_LON2ADD__NO_TOX_NONE_TOY_NONE_TOZ_NONE/lambda1.0//HesseMatrix_sphinx";
    const char *filename_in_uoutcar = "/Users/glensk/Dropbox/Albert/Understanding_distributions/ti/Cu/30__PTS_dosall_from_2x2x2sc__LON_quer_3x3x3kp_all_morse_LONADD__NO_LON2_NONE_LON2ADD__NO_TOX_NONE_TOY_NONE_TOZ_NONE/lambda1.0//u_OUTCAR";
    int columns0=6;
    ...
    if (read_pos==1) {
        ...
        for (i=0;i<(zeitschritte);i++) {
            ...
            //printf("%d %10.3f\n",i,temperature);
            write_dudl(i,file_out_dudl,file_out_dudl_av,read_hesse);
            }
        printf("///////////////////////////////////////////////////////////////////////////////\n");
        ...
        printf("Energy std (DFT-H)   : %6.3f meV/atom (works in general )\n",dudl[zeitschritte-1].dudl_dft_harm_std);
        printf("Energy std (DFT-LA)  : %6.3f meV/atom (works in general lon and tox)\n",dudl[zeitschritte-1].dudl_dft_la_std);
        printf("\n");
        //printf("Forces std <DFT-H>   : %6.3f (eV/A) delta angle degree: \n",sqrt(fdudl_dft_harm_av*1000.));
        printf("Forces std <DFT-H>   : %6.3f (eV/A)\n",sqrt(fdiff_dft_harm_av));
        printf("Forces std <DFT-LA>  : %6.3f (eV/A)\n",sqrt(fdiff_dft_la_av));
        printf("\n");
        printf("Energy dudl (DFT-H)  : %6.3f meV/atom (works in general )\n",dudl_dft_harm_av);
        printf("Energy dudl (DFT-LA) : %6.3f meV/atom (works in general lon and tox)\n",dudl_dft_la_av);
        printf("\n");
        printf("rmin           %5.3f rmin_distmax: %5.3f Angstrom\n",rmin/a0*alat_lattice,alat_lattice/sqrt(2)-rmin/a0*alat_lattice);
        printf("nndist         %5.3f\n",alat_lattice/sqrt(2));
        printf("rmax           %5.3f rmax_distmax: %5.3f Angstrom\n",rmax/a0*alat_lattice,-1.*(alat_lattice/sqrt(2)-rmax/a0*alat_lattice));
