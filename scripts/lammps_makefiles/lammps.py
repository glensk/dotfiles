...
class lammps(object):
  ...
  def __init__(self,name="",cmdargs=None,ptr=None,comm=None):
    ...
    modpath2 = modpath.replace('python','src')
    self.lib = None
    ...
    if not self.lib:
      try:
        if not name: self.lib = CDLL(join(modpath2,"liblammps" + lib_ext),RTLD_GLOBAL)
        else: self.lib = CDLL(join(modpath2,"liblammps_%s" % name + lib_ext),
                              RTLD_GLOBAL)
      except:
          pass
