...
def unwrap_positions(frame,framelast):
    ...
    framelast_wr    = wrap_positions(framelast)
    cell            = frame['cell'].h
    cell_inv        = np.linalg.inv(cell)

    pos_wr           = frame_wr['atoms'].q.copy()
    pos_wr.shape     = (len(pos_wr)/3,3)
    poslast_wr       = framelast_wr['atoms'].q.copy()
    ...
    if q.shape != poslast_uwr.shape:
        poslast_uwr.shape = q.shape
    frame['atoms'].q = (q+poslast_uwr).flatten()
    return frame

def unwrap_positions_new(frame,framelast):
    frame_wr = wrap_positions(frame)
    if type(framelast) == bool:
        return frame
    ...
    while True:
        ...
        try:
            ...
            for trj in trjs_in:
                ...
                if unwrap:
                    frame = unwrap_positions_new(frame,frame_last)
