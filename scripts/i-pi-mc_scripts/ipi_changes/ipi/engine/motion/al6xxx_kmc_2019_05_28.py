...
class AlKMC(Motion):
    ...
    def __init__( self, mode, geop, nstep, a0, ncell, nvac, nsi, nmg,
        ...
        print "self.sites, self.nsites",len(self.sites), self.nsites, "###"
        ...
        print "determines the connectivity of the lattice ..."
        ...
        print('writing KMC_files ...')
        self.kmcfile = open("KMC_AL6XXX","w+")
        self.kmcfile_analyze = open("KMC_analyze","w+")
        self.kmcfile_analyze.write("# column   1     --> step\n")
        self.kmcfile_analyze.write("# column   2     --> time{picosecond} : The elapsed simulation time.\n")
        self.kmcfile_analyze.write("# column   3     --> time{picosecond} : The time of the current timestep.\n")
        #self.kmcfile_analyze.write("# column   4     --> ecurr (Hartree)\n")
        self.kmcfile_analyze.write("# column   4     --> ecurr (meV)\n")
        self.kmcfile_analyze.write("# column   5     --> cdf (sum)\n")
        self.kmcfile_analyze.write("# column   6     --> random number time\n")
    ...
    def step(self, step=None):
        ...
        for i in xrange(len(levents)):
            ...
            print "rates[i]",rates[i],'cdf',cdf,'crates[i]',crates[i]
            self.kmcfile_energies.write("%20.11e"% (levents[i][2]))
            self.kmcfile_barriers.write("%20.11e"% (self.barriers[levents[i][-1]]))
            self.kmcfile_denergies.write("%20.11e"% (-0.5*(ecurr - levents[i][2])))
            self.kmcfile_rates.write("%20.11e"% rates[i])
            self.kmcfile_energies.flush()
            self.kmcfile_denergies.flush()
            self.kmcfile_rates.flush()
            self.kmcfile_barriers.flush()
        self.kmcfile_energies.write("\n")
        self.kmcfile_denergies.write("\n")
        self.kmcfile_rates.write("\n")
        self.kmcfile_barriers.write("\n")
        ...
        NN_1_al = first_NN.count('A')
        NN_1_mg = first_NN.count('M')
        NN_1_si = first_NN.count('S')

        self.kmcfile_analyze.write("%12.0f  %12.4e  %12.4e  %18.11e %12.4e  %8.5f %4.0f %4.0f %4.0f\n"% (step, self.tottime*2.418884326509e-17, dt*2.418884326509e-17, ecurr, cdf, nrand2,NN_1_al,NN_1_mg,NN_1_si ) )
        self.kmcfile_analyze.flush()
        self.tottime += dt
