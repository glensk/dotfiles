...
bohr_to_ang = 0.52917721  # units
bohr_to_ang = 0.5291772105638411  # ase units
hartree_to_ev = 27.211386024367243  # ase units

def h_dot_pos(h,pos):
    s = np.array([])
    for row in pos:
        s = np.append(s,np.dot(h,row))
    s.shape = (len(pos),3)
    #s = np.round(s,12)
    return s


def my_wrap_positions(pos, cell):
    h = cell
    hinv = np.linalg.inv(h)
    s = h_dot_pos(hinv,pos)
    sc = s - np.round(s)
    q = h_dot_pos(h,sc)
    return q

def my_unwrap_positions(pos,poslast, cell):
    h = cell
    hinv = np.linalg.inv(h)
    d = pos - poslast
    s = h_dot_pos(hinv,d)
...
def wrap_positions_ase(positions, cell, pbc=True, center=(0.5, 0.5, 0.5),
                   eps=1e-7):
    """ taken from ase; Wrap positions to unit cell.
...
def write_vacancy_pos_header(cellvec0,step,nvac,filename="vacancy_pos_angstrom.xyz"):
    if step == 0 and os.path.isfile(filename):
        os.remove(filename)
    vac_cellstr = "# CELL(abcABC): "+str(cellvec0)+"  "+str(cellvec0)+"  "+str(cellvec0)+"  60.0 60.0 60.0 Step: "+str(step)+"  Bead: 0 positions{angstrom} cell{angstrom}"
    with open(filename, "a") as myfile:
        myfile.write(str(nvac)+"\n")
        myfile.write(vac_cellstr+"\n")
    return

def write_vacancy_pos(sites,svac,filename = "vacancy_pos_angstrom.xyz",vacpos=False):
    if type(vacpos) == bool:
        vac_posstr =  "      V   " + str(sites[svac][0]*bohr_to_ang)+ "\t" + str(sites[svac][1]*bohr_to_ang)+ "\t" + str(sites[svac][2]*bohr_to_ang)
    else:
        vac_posstr =  "      V   " + str(vacpos[0]*bohr_to_ang)+ "\t" + str(vacpos[1]*bohr_to_ang)+ "\t" + str(vacpos[2]*bohr_to_ang)
    with open(filename, "a") as myfile:
        myfile.write(vac_posstr+"\n")
    return

def write_vacancy_neigh(state,neigh_svac,nivac,step):
    '''
    help
    '''
    ###########################################################
    ## write vacancy neighborhood
    ###########################################################
    filename = "vacancy_neigh_"+str(nivac)+".dat"
    if step == 0 and nivac == 0 and os.path.isfile(filename):
        os.remove(filename)
    n_vac = 0
    n_al = 0
    n_si = 0
    n_mg = 0
    for sneigh in neigh_svac:
        if state[sneigh] == "V" : n_vac+=1
        if state[sneigh] == "A" : n_al+=1
        if state[sneigh] == "M" : n_mg+=1
        if state[sneigh] == "S" : n_si+=1
    with open(filename, "a") as myfile:
        myfile.write(str(step)+" "+str(n_si)+" "+str(n_mg)+" "+str(n_vac)+" "+str(n_al)+"\n")
    return
...
class AlKMC(Motion):
    ...
    def __init__( self, mode, geop, nstep, a0, ncell, nvac, nsi, nmg,
        ...
        self.cellvec0 = self.dcell.h[0,0]*bohr_to_ang
        print('cell')
        print(self.dcell.h)*bohr_to_ang
        self.h = self.dcell.h
        self.hinv = np.linalg.inv(self.dcell.h)
        print('self.h',self.h)
        print('self.hinv',self.hinv)
        #sys.exit()
    ...
    def bind(self, ens, beads, nm, cell, bforce, prng):
        ...
        if with_fakeatom == True:
            self.beads.q[0,:] = self.sites[self.unique_idx(self.state)].flatten() # also sets global q so we can use it further down
        else:
    ...
    def ts_thread(self, ieval, ostr, nstr, nevent, setfev=1):
        ...
        if with_fakeatom == True:
            midx = self.unique_idx_match(list(ostr), list(nstr))
        else:
    ...
    def get_list_of_possible_swaps(self,step,ostr):
        '''  enumerates possible reactive events (vacancy swaps)

        get levents (all the possible swaps (typically 12 in fcc)
        usually 12 events: levents[0] .. levents[11]
        levents[i][0] = svac: index of the current vacancy position
        levents[i][1] = sneigh: self.neigh[svac] -> index of all neighbors
        levents[i][2] = relaxed energy of spapped position
        levents[i][-1] = {A,S,M} Species that swapps
        ...
        ###########################################################
        write_vacancy_pos_header(self.cellvec0,step,self.nvac)
        write_vacancy_pos_header(self.cellvec0,step,self.nvac,filename="ka.txt")

        #vac_cellstr = "# CELL(abcABC): "+str(self.cellvec0)+"  "+str(self.cellvec0)+"  "+str(self.cellvec0)+"  60.0 60.0 60.0 Step: "+str(step)+"  Bead: 0 positions{angstrom} cell{angstrom}"
        #with open("vacancy_pos.xyz", "a") as myfile:
        #    myfile.write(str(self.nvac)+"\n")
        #    myfile.write(vac_cellstr+"\n")

        for nivac,ivac in enumerate(xrange(self.natoms, self.natoms + self.nvac)):
            ...
            write_vacancy_pos(self.sites,svac)
            write_vacancy_neigh(self.state,self.neigh[svac],nivac,step)
    ...
    def step(self, step=None):
        ...
        ### get possible swaps
        levents = self.get_list_of_possible_swaps(step,ostr)

        ### get list of rates from levents
        ...
        for i in xrange(len(levents)):
            ...
            line_new = 'Event %3s %3s %20s >B> %10s >F> %20s' % (i, levents[i][-1], ecurr*hartree_to_ev, round((ets - ecurr)*hartree_to_ev,4), levents[i][2]*hartree_to_ev)
            print line_new
        ...
        q0 = self.sites[self.unique_idx(self.state)].copy()*bohr_to_ang
        ...
        q1 = self.sites[self.unique_idx(self.state)].copy()*bohr_to_ang
        #q1unwrapped = unwrap(q1,q0,self.h)
        print(q0-q1)
        print('---')
        ...
        ### try to wrap unwrap positions
        pos0 = my_wrap_positions(q0, self.h*bohr_to_ang)
        posv = my_unwrap_positions(q1,q0, self.h*bohr_to_ang)[-1]
        write_vacancy_pos(self.sites,svac,filename="ka.txt",vacpos=posv)
        #np.savetxt("pos00.dat",q0)
        #np.savetxt("pos0a.dat",pos0)
        #print('posv',posv)
        #sys.exit()
        #pos1 = wrap_positions(q1, self.h*bohr_to_ang)

        #pos0 = wrap_positions_ase(q0, self.h*bohr_to_ang)
        #pos1 = wrap_positions_ase(q1, self.h*bohr_to_ang)
        #print('q1')
        #print(q1*bohr_to_ang)
        #print('pos')
        #print(pos0*bohr_to_ang)
        #np.savetxt("pos0_unwrapped.dat",q0)
        #np.savetxt("pos0_wrapped.dat",pos0)
        #np.savetxt("pos1_unwrapped.dat",q1)
        #np.savetxt("pos1_wrapped.dat",pos1)
        #sys.exit()
