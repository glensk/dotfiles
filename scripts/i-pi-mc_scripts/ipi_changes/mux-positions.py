...
def unwrap_positions(pos_wr, poslast_wr, poslast_uwr, cell):
    ...
    cell_inv = np.linalg.inv(cell)
    d = pos_wr - poslast_wr
    print('diff pos_wr - poslast_wr')
    print(np.round(d[-2:],8))
    s = cell_dot_pos(cell_inv,d)
    print('jump (1?)')
    print(np.round(s[-2:],8))
    sc = s-np.round(s)
    print('jump (2?)')
    print(np.round(sc[-2:],8))
    q = cell_dot_pos(cell,sc)
    print('jump (3?)')
    print(np.round(q[-2:],8))
    return q+poslast_uwr

#def unwrap_or_wrap(frame,pos_last,idx,args):
def unwrap_or_wrap(frame,pos_last_wr,pos_last_uwr,idx,args):
    ...
    print(idx,'pos last wrapped')
    if type(pos_last_wr) != bool:
        print(np.round(pos_last_wr[-2:],8))
    else:
        print("False")
    print(idx,'pos wrapped')
    print(np.round(pos_wr[-2:],8))

    pos_uwr = False
    if args.unwrap and idx == 0:
        pos_uwr = pos_wr
    elif args.unwrap and idx > 0:
        #print(pos[:10],poslast[:10])
        pos_uwr = unwrap_positions(pos_wr, pos_last_wr, pos_last_uwr, frc['cell'].h)
        print(idx,'pos_uwr')
        print(np.round(pos_uwr[-2:],8))
