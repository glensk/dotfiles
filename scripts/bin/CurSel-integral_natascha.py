...
def SymSelect(datafile, deffile, nsymsel = 20, prefix="cursel", nlandmarks=0, verbose=False,  allel=False, scalemode="none", density=0.0, nsvd=0):
    ...
    nsyms = dict.fromkeys(list(asdef.keys()))
    SF_int = dict.fromkeys(list(asdef.keys()))
    rho = {}
    ...
    print(("Number of symmetry functions for each element: {}".format(list(nsyms.items()))))
    ...
    if allel:
        ...
        if verbose:
            for element in sel1:
                curx = DoCUR(xmats[element], sel1[element], np.asarray(list(range(len(xmats[element]))),int) )            
        ...
        for element in nsyms:
            ...
            if verbose:
                ...
                curx = DoCUR(xmat, sel1[element], np.asarray(list(range(len(xmat))),int))            
