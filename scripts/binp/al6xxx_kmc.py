...
class AlKMC(Motion):
    ...
    def step(self, step=None):
        ...
        for ivac in xrange(self.natoms, self.natoms + self.nvac):
            ...
            cellstr = "# CELL(abcABC): "+str(self.cellvec0)+"  "+str(self.cellvec0)+"  "+str(self.cellvec0)+"  60.0 60.0 60.0 Step: "+str(step)+"  Bead: 0 positions{angstrom} cell{angstrom}"
            posstr =  "      V   " + str(self.sites[svac][0]*bohr_to_ang)+ "\t" + str(self.sites[svac][1]*bohr_to_ang)+ "\t" + str(self.sites[svac][2]*bohr_to_ang)
            with open("vacancy_pos.xyz", "a") as myfile:
                myfile.write(str(self.nvac)+"\n")
                myfile.write(cellstr+"\n")
                myfile.write(posstr+"\n")
