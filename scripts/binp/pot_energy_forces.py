...
class pot_energy_forces_class( object ):
    '''Class to calculate energies and forces from simple pairwise interaction'''
    def __init__(self):
    ...
    def get_or_load_parameters(self):
        ...
        if os.path.isfile(self.datapkl):
            ...
            self.pot_parametrize.loaddatapkl(self.datapkl)
    ...
    def save_information_of_run_add(self,contr = False):
        ...
        #if abs(self.to_dnorm) >= 0.00001:
        #        print "$$$$ to:",self.to_dnorm
        if contr == 'to':
            if abs(self.to_dnorm) >= 0.00001:
                self._info_tvecoutall = np.concatenate((self._info_tvecoutall,np.array([[self.to_dnorm]])),axis=0)
        if contr == 'ti':
            if abs(self.ti_dnorm) >= 0.00001:
                self._info_tvecinsall = np.concatenate((self._info_tvecinsall,np.array([[self.ti_dnorm]])),axis=0)
        return
    ...
    def get_or_load_parameters_lon_to_ti_depending_on_shell(self):
        ...
        if self.u_potadd == 'spline':
            try:
                ...
                self.pot_parametrize = pot_parametrize.forcesneighbors()
                #self.pot_parametrize.loaddatapkl("/Users/glensk/Dropbox/Understand_distributions/displacements_dense/Ir/2x2x2sc_data_3x3x3kp.pkl")
                self.pot_parametrize.loaddatapkl(eval("self.params.u"+str(self.shell)+"nn_potaddpath"))
    ...
    def getef(self, vecnorm, params, pot = False):
        ...
        if pot == 'spline':
            ...
            #ka.loaddatapkl("2x2x2sc_data_3x3x3kp.pkl")
    ...
    def project_vec_into_original_cell(vec,vec0_orig,vec0_orig_outdirection,vec0_curr,vec0_curr_outdirection,forcefunc):
        '''
    ...
    def get_to_ti_direction0_fcc(self):
        ''' input: self.vec0, self.shell
            output: self.todirection0, self.tidirection0
        '''
        # first 13 are not necessary
        ...
        # get dnorm
        #################################################################################
        self.dnorm_p_to = utils.project_vector(self.longvec,self.todirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
        self.dnorm_p_ti = utils.project_vector(self.longvec,self.tidirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
        self.dnorm_r_to = utils.reject_vector(self.longvec,self.todirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
        self.dnorm_r_ti = utils.reject_vector(self.longvec,self.tidirection0) # hat aber die richtige richtung (richtiges vorzeichen da longvec projiziert wird)
        return
    ...
    def get_to_ti_energy_forces(self, contr = False):
        ...
        if self.shell == 2:
            ...
            if contr == 'to':
                self.vec0_orig_formapping = np.array([0.0, cellvecdirnorm, 0.0])   # longvec zum betrachteten atom 4
                self.mappedvec_orig_dir = np.array([1.0, 0.0, 0.0]) # hier entlang haben wir ausgelenkt um die kraft auf atom 8 (in x richtung) zu mappen
            if contr == 'ti':
                self.vec0_orig_formapping = np.array([0.0, 0.0, cellvecdirnorm])   # longvec zum betrachteten atom
                self.mappedvec_orig_dir = np.array([1.0, 0.0, 0.0]) # hier entlang haben wir ausgelenkt um die kraft auf atom 8 (in x richtung) zu mappen
        if self.shell > 2:
            sys.exit("not yet 34")
        ...
        #vec_to_ti_0_r_whole   = utils.reject_vector(self.longvec,self.vec0)
        if contr == 'to':
            vec_to_ti_0     = utils.project_vector(self.longvec,self.todirection0)
            #vec_to_ti_0   = utils.project_vector(vec_to_ti_0_r_whole,self.todirection0)
            potparam = self.u_topx

        if contr == 'ti':
            vec_to_ti_0     = utils.project_vector(self.longvec,self.tidirection0)
            #vec_to_ti_0     = utils.project_vector(vec_to_ti_0_r_whole,self.tidirection0)
            potparam = self.u_tipx

        if pot == False and potparam == False:
            return

        vec_to_ti_length = np.linalg.norm(vec_to_ti_0)
        if vec_to_ti_length == 0.0:
            vec_to_ti_normalized_corr_direction = np.array([0.0,0.0,0.0])
        else:
            vec_to_ti_normalized_corr_direction = vec_to_ti_0/vec_to_ti_length

        self.to_dnorm = 0.0
        self.ti_dnorm = 0.0
        if contr == 'ti':
            self.ti_dnorm = vec_to_ti_length
            self.save_information_of_run_add(contr = 'ti')
        if contr == 'to':
            self.to_dnorm = vec_to_ti_length
            self.save_information_of_run_add(contr = 'to')

        ene_to_ti, fnorm_to_ti = self.getef(vec_to_ti_length,potparam,pot='poly')
        forces_to_ti = vec_to_ti_normalized_corr_direction * fnorm_to_ti

        #if abs(self.to_dnorm) > 0.0:
        #    print "### to:",self.to_dnorm
        #if abs(self.ti_dnorm) > 0.0:
        #    print "### ti:",self.ti_dnorm
        ...
        # a)
        t11_norm, t22_norm = pot_parametrize.get_new_local_basis(longvec = self.longvec, longvec0 = self.vec0_curr, outdirection0 = vec_to_ti_normalized_corr_direction)
        forces_to_ti_orthbasis_a = t11_norm * fnorm_to_ti
        ene_to_ti_a = ene_to_ti

        # b)
        vec_orthbasis_b = utils.project_vector(vec_to_ti_0,t11_norm)
        ene_to_ti_b, fnorm_to_ti_b = self.getef(np.linalg.norm(vec_orthbasis_b),potparam,pot='poly')
        forces_to_ti_orthbasis_b = t11_norm * fnorm_to_ti_b


        #############################################################
        # check that the greatest value is on the same index
        #############################################################
        values = abs(forces_to_ti)
        lvalues = list(values)
        old_i = lvalues.index(max(lvalues))

        values_n = abs(forces_to_ti_orthbasis_a)
        lvalues_n = list(values_n)
        new_i = lvalues_n.index(max(lvalues_n))

        def printinfo():
            print "-------------------------------------ERROR--------------------------------------------------------------"
            print contr+str(self.shell)+": L0:",self.vec0_curr,"L!:",self.longvec,"||",vec_to_ti_normalized_corr_direction,"topnew_norm:",t11_norm,"---> oldF:",forces_to_ti,"newF_a:",forces_to_ti_orthbasis_a,"_b:",forces_to_ti_orthbasis_b
            print "-------------------------------------ERROR--------------------------------------------------------------"
            print "old:",forces_to_ti
            print "new:",forces_to_ti_orthbasis_a
            print "-----------------------------------"
            print "old_i:",old_i,"new_i:",new_i
            print "-----old---------------------------"
            print forces_to_ti[0]
            print forces_to_ti[1]
            print forces_to_ti[2]
            print "-----new a-------------------------"
            print forces_to_ti_orthbasis_a[0]
            print forces_to_ti_orthbasis_a[1]
            print forces_to_ti_orthbasis_a[2]
            print "-----new b-------------------------"
            print forces_to_ti_orthbasis_b[0]
            print forces_to_ti_orthbasis_b[1]
            print forces_to_ti_orthbasis_b[2]
            print "---- ratio a-----------------------"
            print forces_to_ti[old_i]/forces_to_ti_orthbasis_a[new_i]
            print('greatest indizes (value positions) in ols and new basis differ:'+str(forces_to_ti)+"  new:"+str(forces_to_ti_orthbasis_a))
            print "--------------------------------------------------------------------------------------------------------"
            return

        #############################################################
        # check sign of old to force is sign of new to force
        #############################################################
        if contr == 'to':
            if forces_to_ti[old_i] > 0.0 and forces_to_ti_orthbasis_a[old_i] < 0.0:
                printinfo()
                sys.exit('singes differ 1')
            if forces_to_ti[old_i] < 0.0 and forces_to_ti_orthbasis_a[old_i] > 0.0:
                printinfo()
                sys.exit('singes differ 2')

        #if contr == 'to' and abs(forces_to_ti[old_i]) > 0.0001:
        #    if np.sign(forces_to_ti[old_i]) != np.sign(forces_to_ti_orthbasis_a[new_i]):
        #        printinfo()
        #        print('signes of old biggest value and new biggest value differ old:'+str(forces_to_ti)+"  new:"+str(forces_to_ti_orthbasis_a))
        #        sys.exit('signes of old biggest value and new biggest value differ old:'+str(forces_to_ti)+"  new:"+str(forces_to_ti_orthbasis_a))


        #############################################################
        # check that the greatest value are of same sign
        #############################################################
        if abs(forces_to_ti[old_i]) > 0.00001:
            if old_i != new_i:
                if forces_to_ti[old_i]/forces_to_ti_orthbasis_a[new_i] > 1.8:
                    printinfo()
                    sys.exit('greatest indizes (value positions) in ols and new basis differ:'+str(forces_to_ti)+"  new:"+str(forces_to_ti_orthbasis_a))
        ...
        bbb = 0.0
        if ene_to_ti_a != 0.0:
            aaa = round((ene_to_ti_a-ene_to_ti_b)/(ene_to_ti_a)*100.,0)
        if ene_to_ti_b != 0.0:
            bbb = round((ene_to_ti_a-ene_to_ti_b)/(ene_to_ti_b)*100.,0)

        if False: # and self.shell == 2:
            #print "pot:",pot,"potparam:",potparam
            print contr+str(self.shell)+": L0:",self.vec0_curr,"L!:",self.longvec,"||",vec_to_ti_normalized_corr_direction,"topnew_norm:",t11_norm,"---> oldF:",forces_to_ti,"newF_a:",forces_to_ti_orthbasis_a,"_b:",forces_to_ti_orthbasis_b,aaa,bbb

        if contr == 'to':
            #print "forces_ti:",forces_ti
            self.etopx = ene_to_ti
            self.ftopx = forces_to_ti

            self.etopx = ene_to_ti_a
            self.ftopx = forces_to_ti_orthbasis_a
        ...
        if contr == 'ti':
            #print "forces_ti:",forces_ti
            self.etipx = ene_to_ti
            self.ftipx = forces_to_ti

            self.etipx = ene_to_ti_a
            self.ftipx = forces_to_ti_orthbasis_a
            #self.etopx = ene_to_b
            #self.ftopx = forces_to_orthbasis_b


        self.forcestrantopx[self.indi] = self.forcestrantopx[self.indi] + self.ftopx
        self.energytrantopx[self.indi] = self.energytrantopx[self.indi] + self.etopx
        self.forcestrantopy[self.indi] = self.forcestrantopy[self.indi] + self.ftopy
        self.energytrantopy[self.indi] = self.energytrantopy[self.indi] + self.etopy
        self.forcestrantopz[self.indi] = self.forcestrantopz[self.indi] + self.ftopz
        self.energytrantopz[self.indi] = self.energytrantopz[self.indi] + self.etopz

        self.forcestrantipx[self.indi] = self.forcestrantipx[self.indi] + self.ftipx
        self.energytrantipx[self.indi] = self.energytrantipx[self.indi] + self.etipx
        self.forcestrantipy[self.indi] = self.forcestrantipy[self.indi] + self.ftipy
        self.energytrantipy[self.indi] = self.energytrantipy[self.indi] + self.etipy
        self.forcestrantipz[self.indi] = self.forcestrantipz[self.indi] + self.ftipz
        self.energytrantipz[self.indi] = self.energytrantipz[self.indi] + self.etipz
        return
        ...
        for self.indi,self.posi in enumerate(self.positions):
            ...
            for self.shell in self.shells:
                ...
                for self.indj in self.NNlist:
                    ...
                    if self.u_toti == True:
                        self.get_to_ti_direction0_fcc()
                        self.get_to_vec()
                        self.get_to_ti_energy_forces(contr = 'to')
                        self.save_information_of_run_add(contr = 'to')
                        #if abs(self.to_dnorm) > 0.0001:
                        #    print 'kkkkk:',self.to_dnorm
                        self.get_to_ti_energy_forces(contr = 'ti')



                    # to get Fig. 2a distribution
                    #if contr == 'to':
                    #if True:
                    #    self.vec_orig_cell, self.forces_to, ene_to = self.project_vec_into_original_cell(\
    ...
    def dudlrefharmonic(self, dudlnew = False, dudlnewharmonic = False, schritte_up_to = False):
        ...
        self._info_tovec_all = False
        self._info_tivec_all = False
        self._info_lonvec = False
        self._info_tvecoutall= False
        for i in np.arange(schritte):
            ...
            #print '^^^^^^^^^^^^^^^^^^^^^^^^^^^'
            #print self._info_tvecoutall
            #print '^^^^^^^^^^^^^^^^^^^^^^^^^^^'
            self._info_lonvec_all = utils.append_row_to_2d_array(\
                    self._info_lonvec_all,self._info_lonvec)
            self._info_tovec_all = utils.append_row_to_2d_array(\
                    self._info_tovec_all,self._info_tvecoutall)
            self._info_tivec_all = utils.append_row_to_2d_array(\
                    self._info_tivec_all,self._info_tvecinsall)
            ...
            eNEW = self.energymev
            print "listdudl.shape:",listdudl.shape,"i:",i,"enemevtot:",self.energymev,"enemevlon:",energymevlong,"enemevto:",energymevtopx
            ...
            if self.write_vecs_to_disc == True or self.params.save_vecs_to_file_for_DOS == True:
                if i in np.arange(schritte)[::50]:
                    ...
                    np.savetxt("tovec_all",self._info_tovec_all,fmt="%.6f")
                    np.savetxt("tivec_all",self._info_tivec_all,fmt="%.6f")
    ...
    def plot_dudl_pkl(self, whichplot, dudlpkls = False):
        ...
        for i in dudlpkls_all:
            ...
            if whichplot == 'std_conv': # used for ip
                for a in as_:
    ...
    def help(self):
        ...
        p.add_argument('-p', choices=['std_conv', 'std_conv_plot', 'dudl_vs_longvecnorm', 'ene_vs_disp', 'ene_dos', 'dudl_conv'],
                help='plot from pkl file one of the above choices; std_conv (usef for ip tests;)', default=False)
