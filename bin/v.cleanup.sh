...
find . -name "XDATCAR" -exec zpaq a {}.zpaq {} \; -exec rm {} \; # rm is only run when first command successfully
#exit

find . -name "MEMCPU" -exec rm -rf {} \;
for j in `echo "WAVECAR CHG DOSCAR EIGENVAL IBZKPT OSZICAR PCDAT"`;do
    filename=$j
    files=`find . -name "$filename" -exec rm {} \;`
done
echo 1
...
for j in `echo "vasprun.xml OUTCAR"`;do
filename=$j
echo $filename
